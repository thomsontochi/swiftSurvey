<template>
  <div class="sc-ezbkgU Login__Container-sc-5wufj1-0 iYxMPu iEVkHE">
    <div class="Login__Content-sc-5wufj1-1 VxNKD">
      <div class="Login__MainContainer-sc-5wufj1-2 jAcvoZ">
        <a href="#" class="Login__LogoLink-sc-5wufj1-3 rujQf"
          ><img
            src="/static/media/logo.2c489fc4.svg"
            class="Login__LogoImage-sc-5wufj1-4 iyoYoL"
        /></a>
        <div class="Login__MainContent-sc-5wufj1-5 llzQuc">
          <h1 class="Login__Heading-sc-5wufj1-6 fuZfmB">Sign In To Treact</h1>
          <div class="Login__FormContainer-sc-5wufj1-7 gPujhN">
            <div class="Login__SocialButtonsContainer-sc-5wufj1-8 hzwFly">
              <a href="https://google.com" class="Login__SocialButton-sc-5wufj1-9 unjfD"
                ><span class="iconContainer"
                  ><img
                    src="data:image/png;base64,"
                    class="icon"
                    alt="" /></span
                ><span class="text">Sign In With Google</span></a
              ><a href="https://twitter.com" class="Login__SocialButton-sc-5wufj1-9 unjfD"
                ><span class="iconContainer"
                  ><img
                    src="data:image/png;base64,"
                    class="icon"
                    alt="" /></span
                ><span class="text">Sign In With Twitter</span></a
              >
            </div>
            <div class="Login__DividerTextContainer-sc-5wufj1-10 bvGmQx">
              <div class="Login__DividerText-sc-5wufj1-11 cLeyEb">
                Or Sign in with your e-mail
              </div>
            </div>
            <form class="Login__Form-sc-5wufj1-12 gQArSq">
              <input
                type="email"
                placeholder="Email"
                class="Login__Input-sc-5wufj1-13 LlKGX"
              /><input
                type="password"
                placeholder="Password"
                class="Login__Input-sc-5wufj1-13 LlKGX"
              /><button type="submit" class="Login__SubmitButton-sc-5wufj1-14 khMJpg">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  stroke-width="2"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  class="icon"
                >
                  <path d="M15 3h4a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2h-4"></path>
                  <polyline points="10 17 15 12 10 7"></polyline>
                  <line x1="15" y1="12" x2="3" y2="12"></line></svg
                ><span class="text">Sign In</span>
              </button>
            </form>
            <p class="Login___StyledP-sc-5wufj1-17 lfZQJJ">
              <a href="#" class="Login___StyledA-sc-5wufj1-18 fPaNZv"
                >Forgot Password ?</a
              >
            </p>
            <p class="Login___StyledP2-sc-5wufj1-19 MQHgB">
              Dont have an account?
              <a href="#" class="Login___StyledA2-sc-5wufj1-20 iEAyFm">Sign Up</a>
            </p>
          </div>
        </div>
      </div>
      <div class="Login__IllustrationContainer-sc-5wufj1-15 dprhkA">
        <div class="Login__IllustrationImage-sc-5wufj1-16 gJLYTH"></div>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  name: "login",
};
</script>
